...
use Auth;
use Carbon\Carbon;
class SuccessfulbtController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Auth::guard('agent')->check())
        {
            if ($request->wantsJson())
            {
                ...
                $iNetLoadData = Inetloadfdr::where('inetload_from_user_id', $id)->where('inetload_to_user_id', $id)->whereInetloadStatus('2')
            ...
            return view('agent.successfulbt')->with('searchField', $this->searchField);
        }
        else
        {
